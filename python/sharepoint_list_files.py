...
def create_request(order_asc=True):
    ...
    params = {
        ...
        'SortDir' : ('Asc' if order_asc else 'Desc'),
    ...
    return url, params, headers, data

def request_and_parse_response(url, params, headers, data):
    try:
...
def get_sharepoint_files():
    r1 = request_and_parse_response(*create_request(True))
    r2 = request_and_parse_response(*create_request(False))
    combined_files = r1 + r2
    unique_files = []
    seen = set()
...
if __name__ == "__main__":
    ...
    if files:
        ...
        print(f"{len(files)} files saved to {filename}")